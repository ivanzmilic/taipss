...
a) Why does convection transport heat radially outward although there is no net mass flux?

b) Estimate the superadiabatic temperature gradient (\textbf {in order of
magnitude fashion}) in the Sun, \textbf{by} making use of the mixing length
...
\textbf{Explain why we are choosing these specific boundary conditions. (As a question for students).}

\textbf{Solve the Lane-Emden equation for $n=0,1,5$. For these values the equation can be solved \emph{analytically}.}

\textbf{Solve the Lane-Embden equation for $n=1.5$ and $n=3$, and compare the resuls with the ones found in the textbook by D. Prialnik. For this you will have to solve the equation \emph{numerically}.}
