...
\title{Hands-on exercises 8: Degenerate electron gas and properties of white dwarfs}
...
degeneration all the quantum states up to a maximum momentum value are
occupied and Eq.~(\ref{equ:Heisenberg}) becomes equality. Then the
number of electrons with momenta between $(\pmom,\pmom+d\pmom)$ per
unit volume:
\begin{equation}
...
{\bf Problem 2:} Derive the mass-radius relationship of white dwarfs
assuming non-relativistic degenerate electron gas and the
Chandrasekhar mass using the Lane-Emden equation. Hint: recall the for
the equation of state of non-relativistic (relativistic) degenerate
electron gas the polytropic index is $n={3 \over 2}$ ($n=3$), and
assume that hydrogen has been depleted so that $\mu_{\rm e} = 2$.
