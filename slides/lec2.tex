...
\frame{
	...
	\begin{itemize}
		...
		\item We can measure the recieved flux density at the Earth (i.e. irradiance, $\mathcal{E}$). In observational astrophysics we use the term \textbf{apparent magnitude}.
		...
		$$L = F(r=R) = 4\pi R^2 H(r) = \sigma T^4 4\pi R^2$$
		\item Here we call $H(r = R)$ the flux density, or often simply flux ($\rm{W/m^2}$). 
...
\begin{itemize} 
	\item The emissivity (outward flux density at the surface) is equal to: 
	\begin{equation}
	\frac{dE^{\rm em}}{dt dS} = H(r=R) = \sigma T^4
	\end{equation}
	\item Then, the luminosity (emitted energy per second, power, flux) of such object is: 
	$L = 4\pi R^2 \sigma T^4$. 
	\item For the Sun If you substitute the numbers, you will get $T=5777\,\rm{K}$.
	\item This is \textbf{a} temperature. The Sun does not have ``one'' temperature. 
...
\item We leave that for a different course, but in short: 
...
\item After that, multiple other approaches to derivation were made (for example one by Bose and Einstein).
\begin{equation}
...
\begin{itemize}
	...
	\item Combined, these two give us the radius.
...
\frame{
	...
	\begin{itemize}
		...
		\item These parameters will be connected with some differential equations and equation of state. 
		\item Solving them should give us observable parameters ($T_{\rm eff}$, $L$) for given boundary conditions. 
...
\frame{
	...
	\begin{figure}
	\includegraphics[width=5.2cm]{../figures/lec2_sketch1.png}
...
\frame{
	...
	\begin{minipage}{0.39\linewidth}
	\begin{figure}
	\includegraphics[width=5.2cm]{../figures/lec2_sketch3.png}
	\caption*{Expansion of a small element}
	\end{figure}
	\end{minipage}
}
...
\frame{
	...
	\begin{itemize}
		...
		\begin{equation}
		\dot{u} + p {(\frac{1}{\rho})}^. = q - \frac{dF}{dm}
		\end{equation}
	...
	\begin{minipage}{0.39\linewidth}
	\begin{figure}
	\includegraphics[width=5.2cm]{../figures/lec2_sketch2.png}
	\caption*{Energy flow through a star}
	\end{figure}
	\end{minipage}
}
...
\frame{
	...
	\begin{minipage}{0.39\linewidth}
	\begin{figure}
	\includegraphics[width=5.2cm]{../figures/lec2_sketch4.png}
	\caption*{Forces acting on a infinitesimal element}
	\end{figure}
	\end{minipage}
}
...
\frame{
	...
	\begin{itemize}
		...
		\item or in $dm$:
		\begin{equation}
		\frac{dp}{dm} = -G\frac{m}{4\pi r^4}
		\end{equation}
		\item Estimate the pressure at the center:
		\begin{equation}
		p_c = \int_0^M \frac{G m dm}{4\pi r^4} > \int_0^M \frac{G m dm}{4\pi R^4} 
		\end{equation}
...
\frame{
	...
	\begin{itemize}
		...
		\begin{equation}
		\int_{p(0)}^{p(R)} V dP = -\frac{1}{3} \int_0^M \frac{G m\,dm}{r}
		\end{equation}
		\item Here r.h.s. is a third of the total gravitational potential energy of the star - i.e. energy to assemble the star by bringing matter from infinity. Denoted by $\Omega$.
	\end{itemize}
...
\frame{
	...
	\begin{minipage}{0.6\linewidth}
	\begin{itemize}
		\item Ideal gas: $p=nkT$, $u=\frac{3}{2}nkT$.
		\item Or the kinetic energy per unit mass $u=\frac{3}{2} \frac{kT}{m_g}.$
		\begin{equation}
		-3 \int_0^{V(R)} p dV = -3\int_0^M \frac{p}{\rho} dm = \Omega
		\end{equation}
		\item \textbf{From this we can find that} (also valid for gravitational systems like clusters and galaxies and clusters of galaxies): 
		\begin{equation}
		U = -\frac{1}{2}\Omega
		\end{equation}
	\end{itemize}
	...
	\begin{minipage}{0.39\linewidth}
	\begin{figure}
	\includegraphics[width=5.2cm]{../figures/lec2_sketch5.png}
	\caption*{Planets also have roughly $T = -1/2 \Omega$}
	\end{figure}
	\end{minipage}
}
\frame{
	...
	\begin{minipage}{0.6\linewidth}
	\begin{itemize}
		\item Here we will go straight to the equation 2.42 from the book: 
		\begin{equation}
		\dot{E} = \dot{U} + \dot{\cal K} + \dot{\Omega} = L_{nuc} - L
		\end{equation}
		\item In the static case we will have good old $E = U + \Omega = \frac{1}{2} \Omega$.
		\item What with $E$ and $U$ when the star contracts? (Remember that $\Omega$ is negative).
	\end{itemize}
	\end{minipage}
	\hfill
	\begin{minipage}{0.4\linewidth}
...
\frame{
	...
	\begin{itemize}
		\item What with $E$ and $U$ when the star contracts? (Remember that $\Omega$ is negative).
		\item Believe it or not: Total energy decreases, but the temperature of the star \textbf{increases}. 
		\item This is actually not so unbelievable: when you jump from the hill your total energy decreases but your velocity increases. 
	\end{itemize}
	\end{minipage}
	\hfill
	\begin{minipage}{0.39\linewidth}
	\begin{figure}
	\includegraphics[width=5.2cm]{../figures/pvsg.png}
	\caption*{Credits: https://scienceatyourdoorstep.com/}
	\end{figure}
	\end{minipage}
}
...
\begin{itemize}
	\item This is a time that it would take the star to spend all the gas in the process of fusion:
	\begin{equation}
	E = mc^2
	\end{equation}
	\item And then:
	\begin{equation}
	t_{\rm nuc} = \frac{\epsilon M c^2}{L}
	\end{equation}
	\item Which for the Sun is a very large number. Turns out that only a fraction of Sun's hydrogen will participate in the fusion, thus bringing this number down to 10 billion years.
\end{itemize}
}
%
\frame{
...
\begin{itemize}
	\item This is a free-fall time for an object to fall through the star :-) 
	\begin{equation}
	t_{\rm dyn} \approx \frac{R}{v_{esc}} = \sqrt{\frac{R^3}{GM}}
	\end{equation}
	\item For the Sun this amounts to roughly 1000 seconds. As $R$ can vary wildly between the stars, so does the dynamical timescale.
\end{itemize}
}
%
\frame{
\frametitle{What do these timescales tell us}
As a rule of a thumb:
\begin{itemize}

	\item Dynamical timescale tells us that if we perturb the star in the equilibrium, it will restore (hopefully) in $t_{\rm dyn}$. This would also be a characteristic timescale for star formation ($R$ is much much bigger though).

	\item Kelvin-Helmholtz timescale tells us how long it takes to reach the thermal equilibrium globally (locally, temperature changes of course).

	\item The nuclear timescale tells us how much time it would take a star to spend its fuel and enter the next stage of evolution.
\end{itemize}
}
\end{document}
