...
\cos \theta \frac{dI_\nu}{\rho dz} = - \kappa_\nu I_\nu + j_\nu
\end{equation}
show that flux density (energy per unit surface per unit time) is proportional to the gradient of temperature and inversely proportional to the mean opacity $\overline{\kappa}$
}
%
\frame{
...
Dividing the whole equation by $\kappa_\nu$, multiplying by $\cos \theta$ and integrating over all angles will yield: 
\begin{equation}
\frac{4\pi dK_\nu}{\rho \kappa_\nu dz} = - H_\nu
\end{equation}
Integrate over frequencies and divide by $c$ to get the gradient of the radiation pressure on the l.h.s. This will necessitate introduction of a mean opacity. 
\begin{equation}
\frac{1}{\rho \overline{\kappa}}\frac{dp}{dz} = - cH
\end{equation}
As $p = \frac{1}{3}aT^4$, we can easily see that $H\propto \frac{1}{\overline{\kappa}} T^3 \frac{dT}{dz}$
...
%
What are Supernovae Type 1a and why are they important for astrophysics?
%
}
%
\frame{
...
%
Supernovae 1a are an example of the so called ``cataclysmically variable'' stars. They exhibit a rapid, enormous change in brightness, due to the sudden onset of TN reactions through the accretion of material onto the white dwarf in a binary star system. Due to this specific formation mechanism (the explosion happens as soon as a critical mass is reached), they have very well defined absolute brightness, which makes them a standard candle - similar to Cepheids. 
%
}
%
\frame{
...
%
Blackbody is a theoretical concept of the body that is in thermal equilibrium - it is of constant temperature, and all the absorbed energy is emitted back into the space according to Planck's law, that stems from the equlibrium distribution of photons. 

For the majority of their volume, stars are so dense, that the photon mean free path is so short, so that the star locally behaves as a black body. Still, due to temperature gradient, photons are leaking outward. At the surface, stars depart from blackbodies, witnessed by the spectral lines and discontinuities in their spectra, but the general shape of the spectra and differences between the spectral classes can still be understood by an approximation of blackbody radiation.
%
}
%
\frame{
