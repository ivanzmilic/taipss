...
\begin{equation}
F = -4\pi r^2 \frac{4acT^3}{3\overline{\kappa} \rho} \frac{dT}{dr}
\end{equation}
...
\item We will leave that for our winter semester course and now, we will briefly discuss sources of absorption in the stars.
\end{itemize}
}
...
\frametitle{What comes next?}
\begin{itemize}
\item In the execises/homework we will see that: 
\begin{equation}
...
\item Plus, not every wavelength is equally efficient ($n$ is principal quantum number below)!
\begin{equation} 
h\nu \ge \frac{13.6 {\rm eV}}{n^2}
\end{equation}
\end{itemize}
\end{minipage}
\begin{minipage}{0.49\linewidth}
...
\begin{minipage}{0.5\linewidth}
\begin{itemize}
\item After finding the number density of the neutral hydrogen atoms. We have to find the ``populations'' of individual levels:
\begin{equation}
n_i = n_H \frac{g_i e^{-E_i/kT}}{Z}
\end{equation}
\item Where $g_i$ is the statistical weight and Z is the partition function ($Z \approx 2$ for H).
\item Then, for a given wavelength:
\begin{equation}
\chi_\nu = \sum n_i \sigma_i (\nu)
\end{equation}
\end{itemize}
...
%
\frame{
\frametitle{Example: Free-free processes}
\begin{minipage}{0.5\linewidth}
\begin{itemize}
\item Free-free process is a three-body process that involves absorption of emission of a photon. 
\item It is \textbf{NOT} scattering. 
\item Think bremsstralung but also in reverse (i.e. the photon can be spent to accelerate an electron).
\begin{equation}
\chi_\nu = C^* \frac{n_e n^+}{\sqrt{T}} g_{ff} \frac{1}{\nu^3}
\end{equation}
\end{itemize}
\end{minipage}
\begin{minipage}{0.49\linewidth}
\begin{figure}
\includegraphics[width=6cm]{../figures/ff.png}
\caption*{Credit: Walter Maciel, Springer}
\end{figure}
\end{minipage}
}
...
\item Taking into these two but also other opacity sources we should eventually, get the plot on the right
...
\begin{figure}
\includegraphics[width=12cm]{../figures/opacity.png}
\caption*{Credit: Rutten (2001)}
\end{figure}
}
...
\item \q{We will discuss more during July!}
\end{itemize}
}
\end{document}
