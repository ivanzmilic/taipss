...
\begin{itemize}
\item The evolution of the star is driven by its core. \blue{Why?}
\item Nuclear burning happens in the core which changes the
  composition and structure of the rest of the star.
...
\item We anticipate that $T$ and $\rho$ can vary several orders of
  magnitude depending on the star and/or evolutionary stage, so we
...
\item We begin with the equations of state. The most common state is
  ...
  pressure according to (tutorial)
  \begin{equation}
...
\item A given reaction becomes important when $q>q_{\rm min}$
  (e.g.~0.1~J~kg$^{-1}$~s$^{-1}$). This leads to
  \begin{equation}
...
\item When $T$ reaches several billion Kelvin, iron cores start to
  dissociate which plays a significant role at late stages of massive
  stars.
\end{itemize}
\end{minipage}
\begin{minipage}{0.49\linewidth}
...
\item Consider a small radial compression such that the radii are
  ...
  \end{equation}
\item With $\epsilon \ll 1$ the binomial expansion gives:
  \begin{equation}
...
\frametitle{A note on dynamical stability}
\begin{itemize}
\item For relativistic-degenerate electron gas $\gamma = \fourthirds$
  (in reality $\gamma \rightarrow \fourthirds$). This corresponds to
  the Chandrasekhar mass $M_{\rm Ch}\sim 1.46M_\odot$. If the mass is
  higher the star collapses.
\item If radiation pressure becomes dominant such that $p\approx
%($\beta \rightarrow 0$)
  \prad$, we again have $\gamma \rightarrow \fourthirds$.
\item For pure photon gas $p/\rho = u_{\rm rad}/3$. Using the virial
  ...
  This means that the total energy of the star $E = \Omega + u_{\rm
    rad}$ vanishes.
\item This means that the gas sphere is \emph{unbound} and the star
  cannot be held together by gravity.
\end{itemize}
}
%
%
...
\frametitle{A note on dynamical stability}
\begin{itemize}
\item An ionization(-type) process can change the adiabatic index and
  lead to $\gamma < \fourthirds$.
\item The most obvious example is ionization where a single atom can
...
\item Because pressure depends on the number of particles irrespective
  of their nature, ionization/recombination leads to a dependence of
  the pressure on volume (density) that is different than for ideal
  gas.
\item As an example, for a pure singly ionized gas $\gamma <
  ...
  stable.
\item Nuclear processes such as pair production and iron
  photodissociation are further mechanims that can lead to
  $\gamma<\fourthirds$.
\end{itemize}
}
%
...
\item In regions III and IV $\gamma \rightarrow \fourthirds$ due to
  relativistic degeneracy of electrons and radiation presure,
...
%
\frame{
\frametitle{Path of the centre of the star in the ($\log T, \log\rho$) plane}
\begin{minipage}{0.5\linewidth}
\begin{itemize}
\item Consider a polytropic configuration for a star in hydrostatic
  ...
  \end{equation}
  where the coefficient $B_n$ is in the range $0.15$ to $0.2$ for
  polytropic indices $n = 1.5\ldots 3$.
...
\item Combination of all of the information so far can be used to
  sketch the positions of stars with different masses.
...
\item The cores of stars with $M \lesssim 0.08 M_\odot$ are not hot
  enough to ignite nuclear fusion. For $M \gtrsim100 M_\odot$ the
...
\item Stars near $M=M_{\rm Ch}$ cross the carbon detonation very near
  ...
  and collase. \blue{Why is this quite unlikely for a single star?}
...
%%   the ($\log T, \log \rho$) plane.
\item The theory of main sequence needs to reproduce the relations
  (\ref{equ:logL}) and (\ref{equ:Lmass}).
\end{itemize}
\end{minipage}
...
%
\frame{
\frametitle{Theory of main sequence}
\begin{minipage}{0.5\linewidth}
\begin{itemize}
\item $\nu = 3$ compares well with observations of massive stars. This
  is because $\kappa$ is nearly constant for high temperatures where
  $\kappa$ is dominated by eletron scattering.
\item For Kramers opacity law
  \begin{equation}
    \kappa = \kappa_0 \rho T^{-7/2},
  \end{equation}
  the result is $\nu\approx 5$, which is rather good for intermediate
  masses.
\end{itemize}
\end{minipage}
\begin{minipage}{0.49\linewidth}
\begin{figure}
\includegraphics[width=6.5cm]{figures/mass-lum-Malkov_2007_MNRAS_382.png}
\caption*{Credits: Malkov (2007), MNRAS, 382, 1073.}
\end{figure}
\end{minipage}
}
%
...
\item Furthermore, we can now solve for (exercises) $\Rstar$ and
  $\rhostar$:
  \begin{equation}
    \Rstar \propto M^{\frac{n-1}{n+3}},\ \ \ \rhostar \propto M^{2\left(\frac{3-n}{3+n}\right)}.
  \end{equation}
...
\item Furthermore, we can now solve for (exercises) $\Rstar$ and
  ...
  \end{equation}
  \blue{What can be concluded based on these relations?}
\item $n\approx 16$ for the CNO cycle so stellar radius is roughly
  proportional to mass
  \begin{equation}
    \Rstar \propto M.
  \end{equation}
\item For $pp$ chain $n\approx 4$ and
  \begin{equation}
    \Rstar \propto M^{3/7}.
  \end{equation}
  Therefore radius always increases with mass unlike for white dwarfs!
\item Furthermore, since $n>3$ density decreases with increasing $M$
  again opposite to white dwarfs.
\end{itemize}
}
%
%
...
\item There is an observed relation between the luminosity and
  ...
  \end{equation}
  where the slope $\alpha$ is different for low and large $L$.
\item The simple theory matches the observations at least
  qualitatively.
\end{itemize}
\end{minipage}
\begin{minipage}{0.4\linewidth}
...
\frametitle{Theory of the main sequence}
\begin{itemize}
\item The main sequence lifetime is epected to be proportional to the
  nuclear fuel (stellar mass) and inversely proportional to the energy
  release rate (luminosity):
  \begin{equation}
    \tau_{\rm MS} \propto \frac{M}{L} \propto M^{-2}.
  \end{equation}
\item This confirms that more massive stars evolve faster and the main
  sequence shortens gradually from the more massive end.
\end{itemize}
}
%
%
...
\begin{minipage}{0.5\linewidth}
\begin{itemize}
\item Applies to uniform chemical composition: strictly valid for
  \emph{zero-age main sequence}.
\item Convection is ignored.
\item Homogeneity of the star is not a good approximation in advanced
  stages of stellar evolution.
\item More realistic models indicate that mass loss is important,
  e.g., stars with masses much higher than $M_{\rm Ch}$ end up as
  white dwarfs.
\end{itemize}
\end{minipage}
\begin{minipage}{0.49\linewidth}
