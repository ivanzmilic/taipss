...
\item This means that for a \emph{radiative} star
  ...
  \end{equation}
\item This condition can be violated if $\kappa F$ is sufficiently big
  and at least a part of the energy must be transported by means other
  than radiation.
...
itself is already a nearly unsolvable problem (possibly next
lecture). Stellar evolution models require a simple presciption of
convection in 1D (although there is no rigorous way to do this!).
\begin{itemize}
...
\item Assume that half of this goes to the kinetic energy of the fluid element:
  \begin{eqnarray}
    u^2 = (\nabla - \nabla_{\rm ad}) \frac{g\ell^2}{8 \HP}.\label{equ:u2}
  \end{eqnarray}
\item Combining Eqs.~(\ref{equ:deltaT}) and (\ref{equ:u2}) gives the
  convective flux:
  \begin{equation}
    \Fconv = c_P \rho T g^{1/2} \frac{\ell^2}{4\sqrt{2}\HP^{3/2}} (\nabla-\nabla_{\rm ad})^{3/2}.
  \end{equation}
