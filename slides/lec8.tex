...
\item The goal of the modelling efforts is to explain the observed
  ...
  plane, as opposed to, e.g., ($\log \rhoc, \log \TTc$).
\end{itemize}
}
%
%
...
\item If $\overline{\gamma} > \overline{\gamma}_{\rm a}$, the
  ...
  transport massive amounts of energy.
\end{itemize}
}
%
%
...
\item The main-sequence lifetime is related to the amount of nuclear
  ...
  \end{eqnarray}
\item From the homology relations (see Prialnik) we find out that
...
\item Nuclear reactions and degenerate electron gas are an explosive
  combination because of the thermal instability (see Prialnik).
...
\item The outward radiation pressure starts to become important
  especially for the dust grains. The wind picks up and drives strong
  mass loss ($\dot{M} \sim 10^{-4}M_\odot$, \emph{superwind}).
