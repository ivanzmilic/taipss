...
\item The total gas pressure is the sum of (\ref{equ:pI}) and (\ref{equ:pe}):
  \begin{equation}
    p = p_{\rm I} + p_{\rm e} = \left(\frac{1}{\mu_{\rm I}} + \frac{1}{\mu_{\rm e}} \right) {\cal R} \rho T = \frac{{\cal R}}{\mu} \rho T, \ \ \mbox{with}\ \ \frac{1}{\mu} = \frac{1}{\mu_{\rm I}} + \frac{1}{\mu_{\rm e}}. \label{equ:pe}
  \end{equation}
...
\item In 1939 demonstrated (one of) the $pp$-chains that operates in
  the Sun. Around the same time, him and Friedrich von Weisz\"acker
  discovered CNO cycle.
\end{itemize}
}
%
\frame{
...
\item The strong nuclear force keeps the nuclei together and the weak
  ...
  forces on a length scale of the order of $10^{-15}$~m.
\end{itemize}
}
%
%
...
\item The penetration probability is proportional to
  \begin{equation}
...
%
\frame{
\frametitle{CNO cycle}
\begin{itemize}
\item Mediocre shit shit
\end{itemize}
}
%
%
\frame{
\frametitle{Triple $\alpha$ reaction}
\begin{itemize}
\item Awesome shit
\end{itemize}
}
...
\item The first stars got their energy from triple $\alpha$ reactions.
\end{itemize}
\end{minipage}
\hfill
\begin{minipage}{0.59\linewidth}
...
\caption*{Fusion processes as a function of $T$ Credit: \red{PLACEHOLDER}}
\end{figure}
\end{minipage}
}
%
